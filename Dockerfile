...
# redis-server : utilisé uniquement si REDIS_URL n'est pas défini (voir docker-entrypoint.sh).
RUN apt-get update && apt-get install -y --no-install-recommends ffmpeg ca-certificates redis-server && rm -rf /var/lib/apt/lists/*
WORKDIR /app
